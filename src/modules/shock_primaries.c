...
#define BISECTION_ACCURACY 1e-3
		...
		const int nGrid, const double dt, double *Q)
{
	...
	//double dt_particle = SphP_i.Hsml / vshock * Unit.Time;
	//double dt_snap = (Snap.Time - Snap.LastTime) * Unit.Time;
		...
		const double s)
{
	...
	double left = sqrt(2*m_e*k_B*temp_dw) /(m_e*c); // peak of maxwellian
	...
	while (fabs(delta) > BISECTION_ACCURACY) { // root finding
		...
		double n_maxw = maxwellian(temp_dw, p_inj*m_e*c, nth_dw);

		cre_norm = cre_norm_fac / cre_energy_integral(s, p_inj);

		double n_cre = cre_spectrum(cre_norm, p_inj*m_e*c, s);

		delta = (n_maxw - n_cre) / (n_maxw + n_cre); // weighted difference

#ifdef DEBUG
if (SphP_i.ID == 3866445) 
...
#ifdef DEBUG
	printf("Found p_inj=%g CRe_norm=%g \n ", p_inj, cre_norm);
#endif
